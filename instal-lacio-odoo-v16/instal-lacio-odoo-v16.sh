...
function prompt_required {
  ...
  read -p "$prompt_text: ${YELLOW}$default_value${NC} " input_value
...
read -p "Vols continuar amb aquests valors? (s/n): " confirm
if [[ $confirm != "s" ]]; then
  echo -e "${BLUE}Instal·lació cancel·lada.${NC}"
  exit 1
fi
...
function delete_deb_and_sh_files {
  ...
  echo -e "${BLUE}Tots els fitxers .deb i .sh han estat eliminats del directori arrel.${NC}"
}
